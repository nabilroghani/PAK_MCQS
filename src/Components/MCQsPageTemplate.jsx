...
import { FaSearch, FaRegEdit, FaPlayCircle } from "react-icons/fa";
...
const MCQsPageTemplate = ({ 
  ...
  // ✅ Hook hamesha yahan honi chahiye
        ...
        <div className="lg:col-span-8 space-y-8">
            ...
            <div className="relative h-64 md:h-80 group">
              ...
              <div className="absolute inset-0 bg-gradient-to-t from-slate-900 via-transparent to-transparent flex items-end p-8">
                <h1 className="text-white text-3xl md:text-4xl font-black italic tracking-tighter uppercase">
                  {title} <span className="text-cyan-400 underline decoration-2 underline-offset-8">{titleHighlighted}</span> MCQS
                </h1>
              </div>
            </div>
            ...
            <div className="p-6 md:p-8">
                ...
                <div className="overflow-hidden rounded-2xl border border-gray-100 mb-10">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 text-sm">
                    {categories.map((cat, i) => (
                      <Link key={i} to="#" className="p-4 border-b border-r border-gray-50 hover:bg-cyan-50 hover:text-cyan-600 transition-colors flex items-center gap-2 font-semibold text-gray-700">
              ...
              <div className="flex flex-col md:flex-row items-center justify-between gap-6 bg-slate-900 p-6 rounded-2xl shadow-lg border-b-4 border-cyan-500">
                <div className="text-center md:text-left">
                  <h3 className="text-white font-black text-xl mb-1 uppercase">Practice Mode</h3>
                  <p className="text-cyan-400 text-xs font-bold uppercase tracking-widest">Start Quiz to simulate real exam</p>
                </div>
                <div className="flex gap-4">
                    <button className="bg-white hover:bg-gray-100 text-blue-950 px-6 py-3 rounded-xl font-black text-xs uppercase flex items-center gap-2 transition-all active:scale-95 shadow-md"
          ...
          <div className="space-y-6">
            {mcqsData.map((item, index) => (
              <div key={item.id} className="bg-white rounded-[1.5rem] shadow-sm border border-gray-100 overflow-hidden hover:border-cyan-200 transition-all">
                <div className="p-6 md:p-8">
                  ...
                  <h2 className="text-lg md:text-xl font-bold text-blue-950 leading-snug mb-8">{item.question}</h2>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {item.options.map((option, optIndex) => (
                      <div key={optIndex} className="group flex items-center p-4 bg-gray-50 border-2 border-transparent hover:border-cyan-500 hover:bg-cyan-50 rounded-xl transition-all cursor-pointer">
                        ...
                        <p className="text-gray-700 font-bold text-sm">{option}</p>
                      </div>
                    ))}
                  </div>
                </div>
